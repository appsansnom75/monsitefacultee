...
export default function HomePage() {
  ...
  // Gestion de l'animation d'entrée (2.2 secondes)
  useEffect(() => {
  ...
  const definitions = {
    SERI: "IDEAL POUR LES GRANDES QUANTITES : RENDU ECLATANT, RESISTANT ET ECONOMIQUE PAR COULEUR.",
    NUM: "PARFAIT POUR LES LOGOS MULTICOLORES : IMPRESSION DIRECTE DANS LA FIBRE SANS LIMITE DE COULEURS.",
    BROD: "MARQUAGE HAUT DE GAMME : RELIEF ELEGANT ET DURABILITE EXCEPTIONNELLE POUR UN RENDU PRO."
  };

  return (
    <div className="relative min-h-screen bg-[#0f172a] text-white font-sans selection:bg-white selection:text-slate-900 overflow-x-hidden">
      
      {/* 1. ÉCRAN DE CHARGEMENT TYPOGRAPHIQUE */}
      {loading && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-[#0f172a]">
          <h2 className="text-5xl md:text-8xl font-black tracking-[0.3em] uppercase animate-pulse transition-all">
            FACULTEE
          </h2>
        </div>
      )}

      {/* 2. CONTENU DU SITE AVEC TRANSITION */}
      <div className={`transition-all duration-1000 ease-out ${loading ? 'opacity-0 translate-y-4 scale-95' : 'opacity-100 translate-y-0 scale-100'}`}>
          ...
          <div className="bg-white text-slate-900 w-full max-w-lg p-12 rounded-[3rem] relative z-10 shadow-2xl border-t-[10px] border-slate-900 animate-in zoom-in duration-200">
            {!isSubmitted ? (
              ...
              <div className="py-20 text-center animate-in fade-in zoom-in">
                <h3 className="text-3xl font-black uppercase tracking-tighter mb-4">C'EST ENVOYE !</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">MERCI, ON TRAITE VOTRE DEMANDE ET ON VOUS RECONTACTE TRES VITE.</p>
              </div>
            )}
