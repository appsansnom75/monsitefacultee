...
interface ForfaitSeri {
  qte: number;
  couleurs: number[];
}

interface ForfaitSimple {
  qte: number;
  prix: number;
}

interface ForfaitBroderie {
  qte: number;
  emplacements: number[];
}

interface SeriChoice {
  place: string;
  colors: number;
}
...
const FORFAITS_SERIGRAPHIE: ForfaitSeri[] = [
...
export default function HomePage() {
  const [produitId, setProduitId] = useState<string>(PRODUITS[0].id);
  ...
  // Correction des types d'états pour éviter le type "never"
  ...
  // Correction du type "any" sur arr
    ...
    <div className="min-h-screen bg-[#002344] text-white p-6 md:p-12 font-sans selection:bg-white selection:text-slate-900">
        ...
        <div className="grid md:grid-cols-2 gap-16 items-start mb-32">
          ...
          <div className="space-y-4">
            ...
            <div className={`border-2 rounded-3xl transition-all duration-500 ${activeSection === 'support' ? 'border-white bg-white/5' : 'border-slate-800'}`}>
              <button onClick={() => setActiveSection(activeSection === 'support' ? '' : 'support')} className="w-full p-8 flex justify-between items-center text-left">
                <div>
              ...
              {activeSection === 'support' && (
                <div className="px-8 pb-8 animate-in fade-in slide-in-from-top-2">
                  <div className="grid grid-cols-2 gap-3">
                    {PRODUITS.map(p => (
                      <button key={p.id} onClick={() => setProduitId(p.id)} className={`p-4 rounded-xl border-2 transition-all font-black text-[9px] tracking-widest ${produitId === p.id ? 'border-white bg-white text-slate-900' : 'border-slate-800 text-slate-400 hover:border-slate-700'}`}>
                        {p.nom}
                      </button>
                    ))}
                  </div>
            ...
            <div className={`border-2 rounded-3xl transition-all duration-500 ${activeSection === 'marquage' ? 'border-white bg-white/5' : 'border-slate-800'}`}>
              <button onClick={() => setActiveSection(activeSection === 'marquage' ? '' : 'marquage')} className="w-full p-8 flex justify-between items-center text-left">
                <div>
                  <span className="text-[10px] font-black text-slate-500 block mb-1">02. PERSONNALISATION</span>
                  <span className="font-black text-xs tracking-widest uppercase">{seriChoices.length + numChoices.length + broderieChoices.length} MARQUAGE(S)</span>
                </div>
                  ...
                  <div className="p-4 rounded-2xl bg-slate-900/50">
                    ...
                    {seriChoices.map((s, i) => (
                      <div key={i} className="flex gap-2 mb-2">
                        <select className="flex-1 bg-slate-950 p-2 rounded text-[9px] font-black border-none uppercase text-white" value={s.place} onChange={(e) => { const newC = [...seriChoices]; newC[i].place = e.target.value; setSeriChoices(newC); }}>{EMPLACEMENTS.map(e => <option key={e} value={e}>{e}</option>)}</select>
                        <select className="w-20 bg-slate-950 p-2 rounded text-[9px] font-black border-none uppercase text-white" value={s.colors} onChange={(e) => { const newC = [...seriChoices]; newC[i].colors = Number(e.target.value); setSeriChoices(newC); }}>{[1,2,3,4,5,6,7,8].map((n, idx) => <option key={idx} value={idx}>{n} C.</option>)}</select>
                        <button onClick={() => setSeriChoices(seriChoices.filter((_, idx) => idx !== i))} className="text-red-500 px-1">✕</button>
                      </div>
                    ))}
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => setNumChoices([...numChoices, 'COEUR'])} className="p-4 rounded-2xl bg-slate-900/50 text-[9px] font-black uppercase tracking-widest border border-transparent hover:border-white/20">+ Numérique ({numChoices.length})</button>
                    <button onClick={() => setBroderieChoices([...broderieChoices, 0])} className="p-4 rounded-2xl bg-slate-900/50 text-[9px] font-black uppercase tracking-widest border border-transparent hover:border-white/20">+ Broderie ({broderieChoices.length})</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className={`border-2 rounded-3xl transition-all duration-500 ${activeSection === 'quantite' ? 'border-white bg-white/5' : 'border-slate-800'}`}>
              <button onClick={() => setActiveSection(activeSection === 'quantite' ? '' : 'quantite')} className="w-full p-8 flex justify-between items-center text-left">
                <div>
      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="bg-white text-slate-900 w-full max-w-lg p-12 rounded-[3rem] relative z-10 animate-in zoom-in duration-300">
            {!isSubmitted ? (
