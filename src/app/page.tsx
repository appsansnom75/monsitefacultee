...
const FAQS = [
  ...
  { q: "Puis-je voir un aperçu de mon design avant de lancer ?", a: "Oui ! Nous fournissons un aperçu numérique (BAT) de votre design sur le produit choisi avant de lancer la production pour validation finale." },
  { q: "Proposez-vous des produits éco-responsables ?", a: "Oui, nous proposons une sélection de produits en coton biologique et matières recyclées, ainsi que des encres respectueuses de l'environnement." },
];
    ...
    <div className="min-h-screen bg-[#002344] text-white selection:bg-white selection:text-[#002344] font-sans pb-12">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div onClick={() => setSeriChoices([...seriChoices, {place: 'CŒUR', colors: 0}])} className="group cursor-pointer bg-white/5 p-8 rounded-[2.5rem] border border-white/5 hover:border-white/20 transition-all hover:bg-white/[0.08]">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center font-black text-lg group-hover:bg-white group-hover:text-[#002344] transition-all shrink-0">+</div>
                  </div>
        ...
        <footer className="mt-20 border-t border-white/10 pt-20">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-20">
            ...
            <div className="space-y-4">
              ...
              <a href="https://www.instagram.com/facultee.fr" target="_blank" className="block text-sm font-black uppercase hover:text-white transition-colors">Instagram</a>
            </div>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[0.5em] text-white/20 text-center pb-10">© 2026 FACULTEE</p>
        </footer>
      </div>
      ...
      {showPopup && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="bg-white text-[#002344] w-full max-w-2xl p-16 rounded-[4rem] relative z-10 animate-in zoom-in duration-300 shadow-2xl">
            {!isSubmitted ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                <h3 className="text-6xl font-black uppercase tracking-tighter mb-8 italic">L'Iconique.</h3>
                <input type="text" name="Nom" required placeholder="NOM COMPLET / BDE" className="w-full border-b-2 border-[#002344]/10 p-6 font-black uppercase text-sm outline-none focus:border-[#002344]" />
                <input type="email" name="Email" required placeholder="ADRESSE MAIL" className="w-full border-b-2 border-[#002344]/10 p-6 font-black uppercase text-sm outline-none focus:border-[#002344]" />
                <textarea name="Message" placeholder="DÉTAILS DU PROJET (COULEURS, LOGOS, ÉCHÉANCES...)" rows={4} className="w-full border-b-2 border-[#002344]/10 p-6 font-black uppercase text-sm outline-none resize-none focus:border-[#002344]" />
                <button type="submit" className="w-full bg-[#002344] text-white py-8 rounded-[2rem] font-black uppercase text-[13px] tracking-[0.5em] mt-8 hover:scale-[1.01] active:scale-95 transition-all shadow-xl">Envoyer la demande</button>
              </form>
            ) : (
              <div className="py-20 text-center space-y-4">
                <div className="w-16 h-16 bg-[#002344] text-white rounded-full flex items-center justify-center mx-auto text-3xl font-black mb-4">✓</div>
                <h3 className="text-5xl font-black uppercase">Transmis.</h3>
                <p className="text-[12px] font-black uppercase tracking-widest text-[#002344]/40">Nous revenons vers vous sous 24h.</p>
              </div>
            )}
            <button onClick={() => setShowPopup(false)} className="absolute top-12 right-12 text-[#002344]/20 hover:text-[#002344] font-black text-sm uppercase transition-colors">Fermer [✕]</button>
          </div>
        </div>
      )}
